...
def main():
	...
	for url in url_list:
		...
		if len(data) > 0:
			...
			with open('data_average' + os.sep + file_name + '.csv', 'w') as f:
				# write header
	...
	for filename in os.listdir("./data_average"):
		if not filename.startswith('.') and os.path.isfile(os.path.join("./data_average", filename)):

			data = pd.read_csv('./data_average/' + filename, sep=',')
			#print(data.shape )
			data.drop("popularity_percent_current", axis=1, inplace=True)
			#print(data.shape)
			#print('\n')
